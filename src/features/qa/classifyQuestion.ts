...
const categoryKeywords: Array<[QuestionCategory, readonly string[]]> = [
  ['emergency', emergencyKeywords],
  ['health-warning', ['발열', '고열', '열이', '열나', '호흡', '숨쉬', '기침', '건강', '증상', '탈수', '아파', '아픈']],
...
export function hasEmergencyKeyword(question: string): boolean {
  const normalizedQuestion = normalizeQuestionText(question);

  return emergencyKeywords.some((keyword) => normalizedQuestion.includes(normalizeQuestionText(keyword)));
}
...
export function classifyQuestion(question: string): QuestionCategory {
  ...
  for (const [category, keywords] of categoryKeywords) {
    if (keywords.some((keyword) => normalizedQuestion.includes(normalizeQuestionText(keyword)))) {
      return category;
    }
  }
